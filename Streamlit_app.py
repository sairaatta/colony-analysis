...
# Required for Streamlit Cloud / headless servers
matplotlib.use("Agg")
...
# ============================================================
# COLONY ANALYSIS FUNCTION
# ============================================================

def analyze_colony_image(pil_image):
    """
    Runs the full colony detection + analysis pipeline.

    Parameters
    ----------
    pil_image : PIL.Image
        Uploaded Petri dish image.

    Returns
    -------
    dict or None
        Analysis results, or None if no colonies are detected.
    """

    # --------------------------------------------------------
    # Convert PIL image -> OpenCV BGR image
    # --------------------------------------------------------
    ...
    if circles is not None:
        ...
        # Fallback: assume dish is centered
        x = img.shape[1] // 2
        y = img.shape[0] // 2
    ...
    # Remove everything outside Petri dish
    ...
    if len(dish_values) == 0:
        return None

    otsu_threshold, _ = cv2.threshold(
        dish_values.astype(np.uint8),
        0,
        255,
    ...
    for contour in contours:
        ...
        if (
            enclosing_radius < min_radius
            or enclosing_radius > max_radius
        ):
            continue

        # ----------------------------------------------------
        # Filter irregular objects
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # Make sure colony is inside dish
        # ----------------------------------------------------
        ...
        cv2.circle(
            ...
            max(
                1,
                int(enclosing_radius)
            ),
            255,
            -1
        )

        colony_pixels = local_region[
            local_mask > 0
        ]
        ...
        cv2.circle(
            ...
            max(
                2,
                int(enclosing_radius * 1.8)
            ),
            0,
            -1
        )

        background_pixels = local_region[
            background_mask > 0
        ]
        ...
        if (
            len(colony_pixels) > 0
            and len(background_pixels) > 0
        ):

            colony_intensity = float(
                np.mean(colony_pixels)
            )

            background_intensity = float(
                np.mean(background_pixels)
            )
    ...
    plt.title(
        "Colony Circularity Distribution"
    )
    ...
    report = f"""
PETRI-DISH COLONY ANALYSIS REPORT
=================================
...
st.set_page_config(
    page_title="Petri-Dish Colony Analysis",
    layout="wide"
)

st.title(
    "🦠 Petri-Dish Colony Analysis"
)

st.write(
    "Upload a photo of a Petri dish and this tool will "
    "automatically detect colonies (regardless of color), "
    "count them, and measure their size, shape, contrast, "
